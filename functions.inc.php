<?php
    ...
    function joinClan($userID, $clanID){
        ...
        if($numRows == 0){
            ...
            if($query){
                ...
                if($query){
                    echo "<center><b>You have successfully joined ".$clanName."! <a href='./clanpage.php?cid=".$clanID."'>View Clan!</a></b></center>";
                
                }
                
            }
            
        } else echo "Your're already in a clan.";
    	...
    	if(!$rows){
	        ...
	        if(!empty($clanName)&&!empty($clanPass)&&!empty($confirmPass)){
	            ...
	            if($clanPass === $confirmPass){                
			     ...
			     } else echo "Sorry, you must have atleast $".number_format($requiredMoney)." to create a clan.<br />";
	
	        	} else echo "Your passwords did not match.<br />";
	
	    	} else echo "Please fill out the entire form to create a clan.<br />";
	    	
	    } else echo "Sorry, You can only create one clan.<br />";		
    ...
    function generateClansTable($userID, $clanID, $tableName, $enablePagination){
        ...
        if($tableName == "clans"){
            ...
            if($query){
                ...
                echo '<tr><th>Clan Name</th><th>Clan Leader</th><th>Total Money</th><th>Total Experience</th><th>Members</th><th>Options</th></tr>';
                while($clan = mysql_fetch_assoc($query)){
                    ...
                    foreach($clan as $key => $clanInfo){
                        ...
                        switch($key){
                            ...
                            case "joinable";
                                echo '<td><br /><center><form method="post" action='.$joinClan.' style="display:inline; padding:5px"><input type="hidden" name="cid" value="'.$clanID.'"/><input type="submit" name="submit" value=" Join Clan "/></form>  ';
                                echo '<form method="get" action='.$viewClan.' style="display:inline; padding:5px"><input type="hidden" name="cid" value="'.$clanID.'"/><input type="submit" value=" View Clan "/></form></center><br /></td>';
                                break;
            ...
            if(mysql_num_rows($query)){
                echo '<link rel="stylesheet" href="./css/clans.css" type="text/css">';
                echo '<br /><br /><br /><center><table width="98%" cellpadding="0" cellspacing="0" class="t">';
                echo '<tr><th>Members Name</th><th>Money</th><th>Experience</th><th>Options</th></tr>';
                while($clan = mysql_fetch_assoc($query)){
                    echo '<tr>';
                    ...
                    foreach($clan as $key => $clanInfo){
                        ...
                        switch($key){
                            ...
                            case "options";
				if($admin[1]){
					echo '<td><br /><center><form method="post" action="'.$viewUser.'" style="display:inline; padding:5px"><input type="hidden" name="id" value="'.$membersID.'"/><input type="submit" name="submit" value=" View User "/></form>  ';

					echo '<form method="post" action="'.$removeUser.'" style="display:inline; padding:5px"><input type="hidden" name="id" value="'.$membersID.'"/><input type="hidden" name="adminID" value="'.$adminID.'"/><input type="hidden" name="clanID" value="'.$clanID.'"/><input type="submit" value=" Remove User "/></form></center><br /></td>';

					break;
				} else {
					echo '<td><br /><center><form method="post" action='.$joinClan.' style="display:inline; padding:5px"><input type="hidden" name="cid" value="'.$clanID.'"/><input type="submit" value=" Join Clan "/></form>  ';
					echo '<form method="get" action="'.$viewClan.'" style="display:inline; padding:5px"><input type="submit" value=" View Clan "/></form></center><br /></td>';
					break;									
				}
    ...
    function getPages($page, $totalPages){
        echo '<h2 style="display:inline; font-size:150%;">Pages</h2><br />';
